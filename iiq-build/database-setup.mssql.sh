...
export DB_SCHEMA_VERSION=$(sqlcmd -C -N o -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -d identityiq -Q "select schema_version from identityiq.spt_database_version;")
...
then
	...
	if [[ -e /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables.sqlserver ]]; then
		...
		# 8.4+
		sed -ri -e "s/PASSWORD='identityiqah'/PASSWORD='${MSSQL_PASS}'/" /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables.sqlserver

		sqlcmd -C -N o -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -b -i /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables.sqlserver
	else
		...
		# 8.4+
		sed -ri -e "s/PASSWORD='identityiqah'/PASSWORD='${MSSQL_PASS}'/" /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables-${IIQ_VERSION}.sqlserver

		sqlcmd -C -N o -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -b -i /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables-${IIQ_VERSION}.sqlserver
	fi

	echo "=> Finished creating schema objects"
