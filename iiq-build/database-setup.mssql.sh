...
export DB_SCHEMA_VERSION=$(sqlcmd -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -d identityiq -Q "select schema_version from identityiq.spt_database_version;")
...
then
	...
	if [[ -e /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables.sqlserver ]]; then
		...
		sqlcmd -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -b -i /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables.sqlserver
	else
		...
		sqlcmd -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -b -i /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables-${IIQ_VERSION}.sqlserver
	fi
	...
	if [[ -e upgrade_identityiq_tables.sqlserver ]]; then
		...
		sqlcmd -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -i upgrade_identityiq_tables.sqlserver
	else
		for upgrade in `ls upgrade_identityiq_tables-${IIQ_VERSION}*.sqlserver | sort`
		do
			echo "=> Installing upgrade $upgrade"
			sqlcmd -U ${MSSQL_SA_USER} -P ${MSSQL_SA_PASSWORD} -S ${MSSQL_HOST} -i $upgrade
		done
	fi
